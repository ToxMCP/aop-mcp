...
| Effectopedia Export (optional) | Package drafts for Effectopedia import | Operator-configured local export directory | n/a | Local filesystem permissions | Manual run; no uptime guarantee | Generate on demand; ensure the export directory is writable and clean previous artifacts |
| Local Draft Review Artifacts | Persist and inventory scientist-facing review exports on disk | `AOP_MCP_ARTIFACT_OUTPUT_DIR` (default `output/`) | n/a | Local filesystem permissions | On-demand only; no background cleanup | `save_draft_review_artifact` writes under `draft_reviews/` and emits metadata sidecars; `list_saved_draft_review_artifacts` indexes those files for later discovery |
