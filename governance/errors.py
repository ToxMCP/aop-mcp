...
#: The raw source object failed the PRODUCER'S STRICT emission contract (the
#: ``additionalProperties:false`` pydantic ``model_json_schema()`` it serializes
#: through). Synthesized by the source-contract guard at the TOP of run_gate BEFORE
#: any projection; a contract-violating object (e.g. one carrying a field the
#: producer's ``extra=forbid`` model cannot emit) BLOCKS and is NEVER projected. This
#: closes the producer-emission-contract dead-arm class: a "fault" that could only
#: fire a scientific code by carrying a schema-forbidden / undeclared field is caught
#: here as a contract violation instead of silently driving a never-emittable code.
